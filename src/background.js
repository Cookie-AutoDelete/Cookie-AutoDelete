...
export const onDomainChange = (tabId, changeInfo, tab) => {
    if (tab.status === "complete") {
        ...
        if ((typeof tabToDomain[tabId] === 'undefined' || tabToDomain[tabId] === 'newtab')  && mainDomain !== "") {
            ...
            var domainStillOpen = false;
            for (let key in tabToDomain) {
                if (tabToDomain.hasOwnProperty(key)) {
...
var activeTabId = browser.tabs.query({currentWindow: true, active: true})[0];
    ...
    if (details.tabId === activeTabId && details.frameId === 0) {
        ...
        if((mainDomain !== oldDomain) && (typeof oldDomain !== 'undefined') && (oldDomain !== 'newtab')  && (oldDomain !== '')) {
            ...
            if(getSetting(store.getState(), "ecLocalStorageClear")) {
                if ((oldURL.split("://")[0].toLowerCase() === "http") ||
...
var targetPage = "<all_urls>", hstsCount=0, cachedHSTS = [], hstsCacheLength = 200;
    ...
    if ((getSetting(store.getState(), "hstsAlarm")) ||
        ...
        for (let header of e.responseHeaders) {
            if (header.name.toLowerCase() == "strict-transport-security") {
                var host = extractMainDomain(getHostname(e.url)), // returns e.g. "google.de"
                    hstsValue = ((typeof header.value !== 'undefined') ? header.value : header.binaryValue).split("=")[1]; // .split() returns e.g. ["max-age", "3600"]
                ...
                if (getSetting(store.getState(), "hstsAlarm")) {
                    ...
                    for (let hsts of cachedHSTS) {
                        let headerHost = hsts.split("#")[0],
                            headerHSTS = hsts.split("#")[1]; // hsts.split() returns e.g. ["google.de", "3600"]
                       if (host == headerHost) {
                            if (headerHSTS == 0)
                                d++;
                            else
               ...
               //if notification is wanted --> send one
                if (getSetting(store.getState(), "hstsAlarm")) {
                    if(!alertedHSTSHosts.includes(host)) {
                        if (a > 0 && d > 0) {
                            alertHSTS(host);
...
async function alertHSTS (host) {
    var messagePrevented = browser.i18n.getMessage("hstsPreventNotificationText"),
        messageNotPrevented = browser.i18n.getMessage("hstsNotPreventNotificationText");
    var messageAppendix = getSetting(store.getState(), "hstsPrevent") ? messagePrevented : messageNotPrevented;
...
function redirectHSTSRequest(e) {
    var host = extractMainDomain(getHostname(e.url));
	...
	if ((e.url.split("://")[0].toLowerCase() === "http") && (host !== '')) {
        if (getSetting(store.getState(), "hstsPrevent")) {
            ...
            if (a>0) {
				var newURL = "https://" + e.url.split("://")[1];
				return {
					redirectUrl: newURL
				};
            }
...
/*    if (getSetting(store.getState(), "hstsAlarm")) {
        //first let us cache the requests
        var host = extractMainDomain(getHostname(e.url)),
            hstsValue = (e.url.split("/")[0].toLowerCase() === "http:" ? 0 : 1); //if the request is done over HTTP the HSTS value was 0, if HTTPS -> 1
        requestsSent[requestCount] = host + "#" + hstsValue;
        requestCount = ++requestCount % requestCacheLength;

        //iterate over the requests to check for HSTS tracking -> count the activated and deactivated ones
        let a = 0, d = 0;
        for (var request of requestsSent) {
            let reqHost = request.split("#")[0],
                reqHSTS = request.split("#")[1];
            if (reqHost == host) {
                if (reqHSTS == 1)
                    a++;
                else
...
var suffix = 'evercookies-will-be-killed-in-browser-' + browser.runtime.id;
    ...
    if ((typeof url !== 'undefined') && (url !== 'newtab') && (url !== '')) {
        if((url.split("://")[0].toLowerCase() === "http") ||
            ...
            var protocol = url.split("://")[0],
                baseDomain = extractMainDomain(getHostname(url)),
                domain = url.split("/")[2];
            ...
            if (getSetting(store.getState(), "ecLocalStorageClear")) {
                ...
                if (browserDetect() === "Firefox") {
                	...
                	if (!domainChange) {
						...
						//injectScript(protocol, baseDomain);
