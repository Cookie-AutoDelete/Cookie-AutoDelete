		...
		>
			{
				filtered.map((element, index) => {
					...
					const message = browser.i18n.getMessage("notificationContent", [element.recentlyCleaned, summary !== "" ? summary : "(Private)"]);
					return (
							...
							<div className="card-header" id={`heading${index}`}>
								...
								>
									<button className="btn btn-link collapsed" type="button" data-toggle="collapse" data-target={`#collapse${index}`} aria-expanded="false" aria-controls={`collapse${index}`}>
										...
										{`${new Date(element.dateTime).toLocaleString()} - ${message}`}
									</button>
								</h5>
							</div>
