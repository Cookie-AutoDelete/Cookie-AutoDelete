...
describe("StatsService class with no items", () => {
	...
	describe("incrementCounter()", () => {
		...
		describe("incrementCounter() with stat logging off", () => {
			beforeEach(() => {
				browser.storage.local.get.resolves({statLoggingSetting: false});
			});

			it("returns 0 for cookieDeletedCounter for incrementCounter(20)", () => {
				return statsService.incrementCounter(20)
				.then(() => {
					assert.strictEqual(statsService.cookieDeletedCounter, 0);
				});
			...
			it("returns 0 for cookieDeletedCounterTotal for incrementCounter(20)", () => {
				return statsService.incrementCounter(20)
				.then(() => {
					assert.strictEqual(statsService.cookieDeletedCounterTotal, 0);
				});
			...
			it("calls once on browser.storage.local.set for incrementCounter(20) (only at start)", () => {
				return statsService.incrementCounter(20)
				.then(() => {
					assert.isTrue(browser.storage.local.set.calledOnce);
				});
